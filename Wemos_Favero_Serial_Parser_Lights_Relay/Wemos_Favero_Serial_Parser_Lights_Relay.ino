...
const int RedLights = 4; // D2 Brown and Blue Wires
const int GreenLights = 5; // D1 Orange and Green Wires
...
void setup() {
  ...
  pinMode(RedLights, OUTPUT);        // set arduino pin to output mode
  pinMode(GreenLights, OUTPUT);        // set arduino pin to output mode
  ...
  while (Serial.available() > 0) {
    ...
    else if (message_pos == (MAX_MESSAGE_LENGTH - 1))
    {
          ...
          message[5] != prev_message[5] or message[6] != prev_message[6] or message[8] != prev_message[8])
      {
        ...
        // Activates Relays for Red/Green Alcove Lights
